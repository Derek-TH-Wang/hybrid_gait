...
void ConvexMPCLocomotion::run(Quadruped<float>& _quadruped,
  ...
  if(robotMode == 0) {
    ...
    gait->setGaitParam(h,
            Vec4<int>(gaitParam[1], gaitParam[2], gaitParam[3], gaitParam[4]), 
            Vec4<int>(gaitParam[5], gaitParam[6], gaitParam[7], gaitParam[8]), "aio");
    horizonLength = h;
    // std::cout << h << " " << Vec4<int>(gaitParam[1], gaitParam[2], gaitParam[3], gaitParam[4]).transpose() << " " << Vec4<int>(gaitParam[5], gaitParam[6], gaitParam[7], gaitParam[8]).transpose() << std::endl;
  } else {
